...
import Preview from "@/pages/Details/Preview/idnex.vue";
...
export default defineComponent({
  ...
  mounted(){
    this.$store.dispatch('product/getProductInfo',this.$route.params.id);
    this.$store.dispatch('cart/addCart').then(res=>{
      const info = this.$store.state.cart.cartList.find(v=>v.skuId===(this.$route.params.id)/1);
      if(info){
  ...
  methods:{
    ...
    //修改购买数量
    changeByNum:debounce(function(e){
      let num = e.target.value.trim()/1;
      if(goodsNumReg.test(num)){
        this.buyNum = num = this.$store.state.cart.cartList;
      }else{
    ...
    //切换型号
    // changeType(id1,id2,isChecked){
    //
    //   if(isChecked === '1')return;
    //   this.$store.commit('product/CHANGE_CHOOSE_TYPE',{id1,id2})
    // }
      ...
      <div class="conPoin">
        <div class="conPoin">
          <router-link v-for="item in 3" :key="item" :to="{
              path:'/search',
              query:{
                [`category${item}Id`]:categoryView[`category${item}Id`],
                categoryName:categoryView[`category${item}Name`]
              }
            }"
          >
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd @click="$store.commit('product/CHANGE_CHOOSE_TYPE',{s1Id:item.id,s2Id:cg.id})" :class="{active:cg.isChecked==='1'}" v-for="cg in item.spuSaleAttrValueList"

                    :key="cg.id">
                  {{cg.saleAttrValueName}}
                </dd>
              </dl>
            <div class="cartWrap">
              <div class="controls">
                <input @input="changeByNum" autocomplete="off" :value="buyNum" class="itxt">
                <a href="###" :class="['plus',{disabled:buyNum===200}]" @click.prevent="plusOrMins">+</a>
                <a href="###" :class="['mins',{disabled:buyNum===1}]" @click.prevent="plusOrMins">-</a>
              </div>
              <div class="add">
                <a @click.prevent="goCart" href="###" target="_blank">加入购物车</a>
