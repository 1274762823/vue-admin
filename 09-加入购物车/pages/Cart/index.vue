...
<template>
  ...
  <div v-if="$store.state.cart.cartList.length > 0 " class="cart">
      ...
      <div class="cart-body">
        ...
        <ul class="cart-list" v-for="item in $store.state.cart.cartList" :key="item.skuId">
          ...
          <li class="cart-list-con2">
            <img :src="item.imgUrl">
            <router-link :to="'/details/' + item.skuId + '.html'"  class="item-msg">{{ item.skuName }}</router-link>
          </li>
    ...
    <div class="cart-tool">
      ...
      <div class="option">
        <a v-if="$store.state.cart.cartList.filter(v=>v.isChecked===1).length>0" @click="$store.dispatch('cart/deleteCheckedAsync')" href="#none">删除选中的商品</a>
      </div>
      <div class="money-box">
